...
export function ProductDescription({ product }: { product: Product }) {
  let amount = 0,
    lowestAmount = 0,
    highestAmount = 0
  const priceField = 'priceInINR' as keyof Product
  const hasVariants = product.enableVariants && Boolean(product.variants?.docs?.length)

  if (hasVariants) {
    const priceField = 'priceInINR' as keyof Variant
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="uppercase font-mono">
          {hasVariants ? (
            <Price currencyCode="INR" highestAmount={highestAmount} lowestAmount={lowestAmount} />
          ) : (
            <Price amount={amount} currencyCode="INR" />
          )}
        </div>
      </div>
      {product.description ? (
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        <Suspense fallback={null}>
          <AddToCart buyNow product={product} />
        </Suspense>
